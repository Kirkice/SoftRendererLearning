...
Mesh* mesh_create(char* meshname, char* texname)
{
	...
	matrix4x4_identity(&mesh->matrix_world);

	char* result = (char*)malloc(strlen(RESOURCES_PATH) + strlen(meshname) + 1);
...
{
	matrix4x4_identity(&mesh->matrix_world);
	matrix4x4_rotation(&mesh->matrix_world, mesh->rotation.x, mesh->rotation.y, mesh->rotation.z);
